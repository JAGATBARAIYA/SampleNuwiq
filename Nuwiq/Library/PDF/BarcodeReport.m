...
- (void)drawRect:(CGRect)rect
{
    @try
    {
        [self createPDFFile:@"BarcodeReport.pdf"];
...
- (void)writeInPDF:(CGRect)aRect
{
    @try
    {
        ...
        [self writeText:@"1.) Barcode Log" ForXpos:intXPos ForYPos:intYPos ForFont:PDFDEFAULTS_TEXT_TITLE_BOLD ForAlignment:NSTextAlignmentLeft ForSize:(CGSize){572,20} ForColor:FONT_RED];
        intYPos += 40;
        ...
        if (aMutArray.count > 0)
        {
            for (NSDictionary *aDict in aMutArray)
            {
                ...
                if ([aDict[@"isReport"] intValue] == 1)
                    intYPos = [self CreateRowWithDate:aDict[@"date"] Time:aDict[@"time"] Data:aDict[@"data"] YPosition:intYPos RowHeight:intRowHeight];
            }
        }
        else
        {
            [self NoRecordFoundWithMessage:@"Barcode Data Not available!" YPosition:intYPos RowHeight:intRowHeight];
        }
...
- (int)CreateRowWithDate:(NSString *)strDate Time:(NSString *)strTime Data:(NSString *)strData YPosition:(int)intYPosition RowHeight:(int)rowHeight
{
